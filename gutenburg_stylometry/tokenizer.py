"""
Tokenizer for 19th-century English literature.

Optimized for Dickens, Austen, Brontë, and similar Victorian/Regency texts.
Handles period-specific orthography, Gutenberg OCR artifacts, and dialect representation.
"""

import re
from typing import Iterator

from gutenburg_stylometry.protocols import Tokenizer as TokenizerProtocol
...
_MULTI_CHAR_PATTERN = re.compile(
    "|".join(re.escape(k) for k in _MULTI_CHAR_REPLACEMENTS))
...
# =============================================================================
# GUTENBERG ARTIFACT REMOVAL
# =============================================================================

# Italics/emphasis markers: _word_, ALLCAPS preserved but markers stripped
...
# Matches hyphen at end of line followed by continuation
_LINEBREAK_HYPHEN_PATTERN = re.compile(r"(\w+)-\s*\n\s*(\w+)")


def clean_gutenberg_artifacts(text: str) -> str:
    """Remove Project Gutenberg formatting artifacts."""
...
#
# 1. Apostrophe-initial contractions: 'twas, 'tis, 'twere, 'em, 'gainst
# 2. Internal elisions: e'er, ne'er, o'er, th'
...
# 6. Dialect g-dropping: runnin', somethin', nothin'
# 7. Plain words
# 8. Roman numerals (chapter headings): VII, XLII, MCMXCIX
# 9. Ordinals: 1st, 2nd, 3rd, 22nd
# 10. Decimal numbers: 3.14, 1,000
# 11. Plain integers

_TOKEN_PATTERN = re.compile(
    r"""
    '[a-z]+                                     # 'twas, 'tis, 'em, 'gainst
    |
    [a-z]+'[a-z]+                               # e'er, ne'er, o'er, th'abbey
    |
...
class VictorianTokenizer:
    """
    Tokenizer optimized for 19th-century English literature.

    Handles Victorian orthographic conventions, dialect representation,
    and Project Gutenberg formatting artifacts.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            min_length: Minimum token length (applied during extraction)
    ...
    def tokenize(self, text: str) -> list[str]:
        ...
        text = clean_gutenberg_artifacts(text)
...
assert isinstance(VictorianTokenizer(), TokenizerProtocol)
...
def tokenize(text: str, lowercase: bool = True) -> list[str]:
    ...
    return VictorianTokenizer(lowercase=lowercase).tokenize(text)


def tokenize_iter(text: str, lowercase: bool = True) -> Iterator[str]:
    """Convenience function for lazy tokenization."""
    return VictorianTokenizer(lowercase=lowercase).tokenize_iter(text)
